...
Group by category (geospatial, financial, company, demographics, weather, health, transport, energy, education, property, identity, crime, reference) using the trigger keywords from the existing datascout reference (see `.arckit/agents/READER-PATTERN.md` for the trigger map).
