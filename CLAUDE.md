...
7. **Paperclip plugin** (`arckit-paperclip/`) — TypeScript plugin for Paperclip AI, built via `npm run build`; not generated by the converter, maintained separately
...
# Build Paperclip plugin (TypeScript → dist/)
cd arckit-paperclip && npm install && npm run build

# Run Python test suite
pip install pytest && pytest tests/
# Run a single Python test file
pytest tests/plugin/test_commands_structure.py -v
# Run JavaScript hook/handoff tests
...
`scripts/python/`: Python equivalents of the bash helpers (`create-project.py`, `generate-document-id.py`, `list-projects.py`, `migrate_classification.py`) — used when bash is unavailable.
...
5. If the command emits a structured handoff payload, add a JSON schema to `arckit-claude/schemas/` and a corresponding test in `tests/plugin/`
6. Run `python scripts/converter.py` to generate Codex/OpenCode/Gemini/Copilot variants
7. Test plugin in a test repo, test CLI via `arckit init test --ai codex --no-git`
8. Update README.md, docs/index.html, docs/DEPENDENCY-MATRIX.md, CHANGELOG.md
