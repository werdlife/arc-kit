...
- Validator script available: ensure `~/.gemini/extensions/arckit/scripts/validate-handoff.mjs` exists via `Read`. Do not try to load it via `node -e require(...)` — it is an ESM module.
